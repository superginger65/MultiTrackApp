...
let trackToPlay = `Tracks/Audio 16 Duet 14 Simple Song 4 Mix with Met.mp3`;
let track4Mute = false;
  ...
  function toggleTrack(trackId, shouldPlay) {
    ...
    if (!track4Mute) {
      trackToPlay = `Tracks/Audio 16 Duet 14 Simple Song 4 Mix with Met.mp3`; 
    } else {
      trackToPlay = `Tracks/Audio 16 Duet 14 Simple Song 4 Mix no Met.mp3`;
    }
    if (shouldPlay) {
