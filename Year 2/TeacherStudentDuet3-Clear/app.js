...
let trackToPlay = `Tracks/Audio 4 Duet 3 Clear Mix with met.mp3`;
let track4Mute = false;
  ...
  function toggleTrack(trackId, shouldPlay) {
    ...
    if (!track4Mute) {
      trackToPlay = `Tracks/Audio 4 Duet 3 Clear Mix with met.mp3`; 
    } else {
      trackToPlay = `Tracks/Audio 4 Duet 3 Clear Mix no met.mp3`;
    }
    if (shouldPlay) {
