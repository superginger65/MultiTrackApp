...
let trackToPlay = `Tracks/Audio 17 Duet 15 Simple Song 5 Mix with Met.mp3`;
let track4Mute = false;
  ...
  function toggleTrack(trackId, shouldPlay) {
    ...
    if (!track4Mute) {
      trackToPlay = `Tracks/Audio 17 Duet 15 Simple Song 5 Mix with Met.mp3`; 
    } else {
      trackToPlay = `Tracks/Audio 17 Duet 15 Simple Song 5 Mix no Met.mp3`;
    }
    if (shouldPlay) {
